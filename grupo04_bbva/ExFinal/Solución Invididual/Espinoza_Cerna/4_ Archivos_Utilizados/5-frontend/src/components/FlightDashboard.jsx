...
const FlightDashboard = () => {
  ...
  const [filters, setFilters] = useState({ airlines: [], origins: [], dates: [] });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/flights/kpis?${params}`);
  ...
  return (
    <div className="bg-gray-900/50 border border-gray-700 rounded-xl p-6 backdrop-blur-sm">
      <h2 className="text-2xl font-bold text-white mb-4 font-orbitron">Dashboard de KPIs de Vuelos</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {renderFilterDropdown('airline', 'Aerolínea', filters.airlines)}
        {renderFilterDropdown('origin', 'Aeropuerto de Origen', filters.origins)}
        {renderFilterDropdown('date', 'Fecha', filters.dates)}
      </div>
            ...
            <table className="w-full text-left font-mono text-sm">
              <thead className="border-b border-gray-600 text-white/70 uppercase">
                <tr>
                  ...
                  <th className="p-3 text-right">Vuelos Totales</th>
                  <th className="p-3 text-right">Llegada a Tiempo (%)</th>
                  <th className="p-3 text-right">Cancelaciones (%)</th>
                  <th className="p-3 text-right">Retraso Prom. (min)</th>
                </tr>
              </thead>
              <tbody className="text-white">
                {data.map((row, index) => (
                  <tr key={index} className="border-b border-gray-800 hover:bg-gray-800/50">
                    ...
                    <td className="p-3 text-right">{row.total_flights}</td>
                    <td className="p-3 text-right text-green-400">{row.on_time_arrival_pct?.toFixed(2)}</td>
                    <td className="p-3 text-right text-yellow-400">{row.cancellation_rate_pct?.toFixed(2)}</td>
                    <td className="p-3 text-right text-red-400">{row.avg_arrival_delay?.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
