...
# ==========================================
# 0. PASO PREVIO: ENRIQUECER DATA
# ==========================================
col_fecha = buscar_columna(df_silver, ["date", "flightdate", "flight_date"])

df_gold_base = df_silver.withColumn("date_obj", to_date(col_fecha)) \
...
# Dim Tiempo
dim_tiempo = df_gold_base.select("year", "month", "day_of_month", "day_of_week") \
    .distinct() \
...
catalog_airports = df_gold_base.select(col_origin.alias("airport_code")) \
...
# ----------------------------------------------------------
# KPIs adicionales (para pegar debajo del bloque que ya tienes)
# ----------------------------------------------------------
...
kpis_gold = fact_vuelos.groupBy("carrier_code").agg(
    ...
    # 2) Porcentaje de vuelos a tiempo (%): llegada <= 0 minutos
    (count(when(col("arr_delay") <= 0, True)) / count("*") * 100).alias("porcentaje_a_tiempo_pct"),

    # 3) Retraso promedio SOLO cuando hubo retraso (arr_delay > 0)
    avg(when(col("arr_delay") > 0, col("arr_delay"))).alias("retraso_promedio_solo_retrasados"),

    # 4) Minutos totales de retraso por causa (para identificar la causa dominante)
...
kpis_gold.write.format("delta").mode("overwrite").saveAsTable("kpis_reporte_gold")
print("✅ Tabla de KPIs generada: kpis_reporte_gold.")
